...
这次请严格按“先源码、再 tag、最后 Release 附件”的顺序。`v1.0.8` 安装包较大，
...
- 外层 `GitHub上传材料-v1.0.8.zip`：只是给你保存和搬运整套材料，不上传。
...
7. 每批上传完成后，提交说明填：`Release v1.0.8 source`。
...
## 第二步：创建 v1.0.8 Release

1. 打开 <https://github.com/DLBorjigin/zanmenliaotian-ai-exporter/releases/new>。
2. 点 `Choose a tag`，输入 `v1.0.8`。
3. 选择 `Create new tag: v1.0.8 on publish`。
4. `Target` 选择刚上传完源码的 `main`。
5. `Release title` 填：`微信聊天导出工具 v1.0.8`。
6. 把本材料根目录的 `发布说明-v1.0.8.md` 全文复制到说明框。
7. 在附件框只拖入：
   `02-Release附件/微信聊天导出工具-v1.0.8-Windows.zip`。
...
- 标题和 tag 都是 `v1.0.8`；
- 页面有绿色 `Latest`；
- Assets 中有 `微信聊天导出工具-v1.0.8-Windows.zip`；
- 安装包大小应明显大于旧版本，因为包含约 141 MiB 的模型；
- 点进 `v1.0.8` tag 后，`pyproject.toml` 显示 `1.0.8`；
...
如果已经创建了 v1.0.8 tag 却尚未上传完源码，请先暂停发布并让维护者检查 tag 指向；不要把新安装包挂在旧源码 tag 下。
